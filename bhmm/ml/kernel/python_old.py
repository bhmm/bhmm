...
def forward(A, B, pi, ob, dtype=np.float32):
    ...
    for t in range(T-1):
        ...
        alpha[t+1,:] = np.dot(alpha[t,:], A) * B[:, ob[t+1]]
        # scaling factor
...
def backward(A, B, ob, dtype=np.float32):
    ...
    for t in range(T-2, -1, -1):
        ...
        beta[t,:] = np.dot(A, beta[t+1,:] * B[:,ob[t+1]])
        # scaling factor
...
def transition_probabilities(alpha, beta, A, B, ob, dtype=np.float32):
    ...
    for t in range(T-1):
        ...
        xi[t,:,:] = np.dot(alpha[t,:][:,None] * A, np.diag(B[:,ob[t+1]] * beta[t+1,:]))
...
def transition_counts(alpha, beta, A, B, ob, dtype=np.float32):
    ...
    xi = transition_probabilities(alpha, beta, A, B, ob, dtype=dtype)
...
def main():
    ...
    pi = np.array([0.5,0.5])
    ob = np.zeros((1000), dtype=np.float16)
    ob[250:750] = 1
    ...
    for k in range(N):
        ll2[k], alpha, scaling = forward(A, B, pi, ob)
        beta = backward(A, B, ob)
        gamma = state_probabilities(alpha, beta)
        xi = transition_probabilities(alpha, beta, A, B, ob)
        A, B, pi = update(gamma, xi, ob, len(B[0]))
    ...
    # ob = np.zeros((1000), dtype=np.float16)
    # ob[500:] = 1
    # (logprob, alpha, scale) = forward(A, B, pi, ob)
    # print logprob
    # print alpha
    # print scale
    #
    # beta = backward(A, B, ob, scale)
    # print beta
    #
    # beta = backward_scaled(A, B, ob)
    # print beta
